...
class ODERegression(BaseModel):
    def __init__(self, args, device):
        ...
        """
        self.uniform_timestep = getattr(args, "uniform_timestep", False)
        self.attn_mask = getattr(args, "attn_mask", True)
        ...
        self.diffusion_forcing = getattr(args, "diffusion_forcing", False)
    ...
    @torch.no_grad()
    def _prepare_generator_input(self, ode_latent: torch.Tensor, is_force_data: bool) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def generator_loss(
        ...
        is_force_data = initial_latent is not None

        noisy_input, timestep = self._prepare_generator_input(
            ode_latent=ode_latent, is_force_data=is_force_data)
